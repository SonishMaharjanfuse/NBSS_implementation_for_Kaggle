...
class SS_SemiOnlineDataModule(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        if self.clean_speech_dataset == 'wsj0' or self.clean_speech_dataset == 'wsj0-mix-4spk':
            print("?????????????????????????????")
            spk1_cfgs, spk2_cfgs, spk3_cfgs, spk4_cfgs, spk5_cfgs = SS_SemiOnlineDataModule.prepare_data_wsj0_mix(f'/content/drive/MyDrive/NBSS_5_speaker')
    ...
    def prepare_data_wsj0_mix(config_dir: str,) -> Tuple[Dict[str, List[Dict[str, str]]], Dict[str, List[Dict[str, str]]]]:
        ...
        if os.path.exists(wav_cfg_path):
            ...
            for key, dataset in {'train': 'tr', 'validation': 'cv', 'test': 'tt'}.items():
                df = pd.read_csv(f'{config_dir}/mix_5_spk_' + dataset + '.txt', delim_whitespace=True, names=['wav_file1', 'scale1', 'wav_file2', 'scale2','wav_file3', 'scale3', 'wav_file4', 'scale4', 'wav_file5', 'scale5'])
                # print(df)
                # print(key)
                ...
                for i in range(num_files):
                    ...
                    inwav1_name, inwav2_name, inwav3_name, inwav4_name, inwav5_name = dfi[0], dfi[2], dfi[4], dfi[6],  dfi[8]  #'wsj0/si_tr_s/40n/40na010x.wav'
                    inwav1_snr, inwav2_snr, inwav3_snr,inwav4_snr, inwav5_snr = dfi[1], dfi[3], dfi[5], dfi[7], dfi[9]
                    ...
                    spk_name5 = inwav5_name.split('/')[3][0:3]
                    ...
                    gender_i5 = genders.ID.values.searchsorted(spk_name5, side='left')
                    ...
                    gender5 = genders.iloc[gender_i5, 1]                    
                    ...
                    assert spk_name5 == genders.iloc[gender_i5, 0], 'error1'                    
                    ...
                    spk5_cfg.append({'wav': inwav5_name, 'speaker': spk_name5, 'gender': gender5, 'dataset': f"wsj0/{key}"})
                # print(spk3_cfg)
                spk1_cfgs[key] = spk1_cfg
                spk2_cfgs[key] = spk2_cfg
                spk3_cfgs[key] = spk3_cfg
                spk4_cfgs[key] = spk4_cfg
                spk5_cfgs[key] = spk5_cfg                
            rank_zero_info(f'write wav configuration to {wav_cfg_path}')
            f = open(wav_cfg_path, 'w', encoding='utf-8')
            json.dump({'spk1_cfgs': spk1_cfgs, 'spk2_cfgs': spk2_cfgs, 'spk3_cfgs': spk3_cfgs, 'spk4_cfgs': spk4_cfgs,'spk5_cfgs': spk5_cfgs}, f, indent=4)
            f.close()

        return spk1_cfgs, spk2_cfgs, spk3_cfgs, spk4_cfgs, spk5_cfgs
        ...
        if stage is not None and stage == 'test':
            if self.clean_speech_dataset == 'wsj0-mix-4spk':
                ...
                self.train = SS_SemiOnlineDataset(
                    speeches=[self.spk1_cfgs['train'], self.spk2_cfgs['train'], self.spk3_cfgs['train'], self.spk4_cfgs['train'], self.spk5_cfgs['train']],
            ...
            self.val = SS_SemiOnlineDataset(
                speeches=[self.spk1_cfgs['validation'], self.spk2_cfgs['validation'],self.spk3_cfgs['validation'], self.spk4_cfgs['validation'],self.spk5_cfgs['validation']],
            ...
            self.test = SS_SemiOnlineDataset(
                speeches=[self.spk1_cfgs['test'], self.spk2_cfgs['test'], self.spk3_cfgs['test'], self.spk4_cfgs['test'], self.spk5_cfgs['test']],
                rirs=self.rirs['test'],
                speech_overlap_ratio=self.speech_overlap_ratio,
                speech_scale=self.speech_scale,
                audio_time_len=self.audio_time_len_for_test,
